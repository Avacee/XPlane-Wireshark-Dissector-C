...
static const value_string xplane_BeaconTypeLookup[4] = {
    { 1, "Master" },
    { 2, "External Visual" },
    { 3, "IOS" },
    { 0, NULL }
...
static const value_string xplane_StartTypeLookup[] = {
    { 5  , "RepeatLast" },
    { 6  , "LatLong" },
    { 7  , "GeneralArea" },
    { 8  , "NearestAirport" },
...
{
    if (tvb_captured_length(tvb) != 165)
        return 0;

    proto_item* xplane_acfn_item;
    proto_tree* xplane_acfn_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.acfn");

    xplane_acfn_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_acfn_item, ": Load Aircraft");

    xplane_acfn_tree = proto_item_add_subtree(xplane_acfn_item, ett_xplane_acfn);
    proto_tree_add_item(xplane_acfn_tree, hf_xplane_acfn_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_acfn_tree, hf_xplane_acfn_index, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(xplane_acfn_tree, hf_xplane_acfn_path, tvb_content, 4, 150, ENC_ASCII);
...
{
    if (tvb_captured_length(tvb) != xplane_ACPR_PACKET_LENGTH)
        return 0;

    proto_item* xplane_acpr_item;
    proto_tree* xplane_acpr_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.acpr");

    xplane_acpr_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_acpr_item, ": Load and Position Aircraft");

    xplane_acpr_tree = proto_item_add_subtree(xplane_acpr_item, ett_xplane_acpr);
    proto_tree_add_item(xplane_acpr_tree, hf_xplane_acpr_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_acpr_tree, hf_xplane_acpr_index, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(xplane_acpr_tree, hf_xplane_acpr_path, tvb_content, 4, 150, ENC_LITTLE_ENDIAN);
...
static int dissect_xplane_alrt(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_ACPR_PACKET_LENGTH)
        return 0;

    proto_item* xplane_alrt_item;
    proto_tree* xplane_alrt_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.alrt");

    xplane_alrt_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_alrt_item, ": Display Alert Message");

    xplane_alrt_tree = proto_item_add_subtree(xplane_alrt_item, ett_xplane_alrt);
...
{
    if (pinfo->destport != xplane_pref_becn_port)
        return 0;

    proto_item* xplane_becn_item;
    proto_tree* xplane_becn_tree;
    guint32 major, minor;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.becn");
    //    col_clear(pinfo->cinfo, COL_INFO);
    //    col_set_str(pinfo->cinfo, COL_INFO, "X-Plane (BECN)");

    xplane_becn_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_becn_item, ": Beacon Packet");

    xplane_becn_tree = proto_item_add_subtree(xplane_becn_item, ett_xplane_becn);
    ...
    proto_tree_add_item(xplane_becn_tree, hf_xplane_becn_name, tvb_content, 16, -1, ENC_ASCII);

    if (major == 1 && minor == 2)
    {
        proto_tree_add_item(xplane_becn_tree, hf_xplane_becn_newport, tvb_content, 16 + tvb_strsize(tvb_content, 21), 2, ENC_LITTLE_ENDIAN);
    }
    return tvb_captured_length(tvb);
}
...
{
    proto_item* xplane_cmnd_item;
    proto_tree* xplane_cmnd_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.cmnd");

    xplane_cmnd_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_cmnd_item, ": Command Message");

    xplane_cmnd_tree = proto_item_add_subtree(xplane_cmnd_item, ett_xplane_cmnd);
    proto_tree_add_item(xplane_cmnd_tree, hf_xplane_cmnd_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_cmnd_tree, hf_xplane_alrt_line1, tvb_content, 0, -1, ENC_ASCII);
...
{
    if (((tvb_captured_length(tvb) - 5) % 36) != 0)
        return 0;

    proto_item* xplane_data_item;
    proto_tree* xplane_data_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 36;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.data");
    //    col_clear(pinfo->cinfo, COL_INFO);
    //    col_set_str(pinfo->cinfo, COL_INFO, "X-Plane (DATA)");

    xplane_data_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_data_item, ": DATA");

    xplane_data_tree = proto_item_add_subtree(xplane_data_item, ett_xplane_data);
    proto_item* xplane_header_item = proto_tree_add_item(xplane_data_tree, hf_xplane_data_header, tvb, 0, 4, ENC_ASCII);
    proto_item_append_text(xplane_header_item, ", Record Count: %d", recordCount);

    for (guint32 i = 0; i < recordCount; i++)
    {
        gint32 index = tvb_get_gint32(tvb_content, 36 * i, ENC_LITTLE_ENDIAN);
        proto_tree* xplane_content_tree = proto_tree_add_subtree_format(xplane_data_tree, tvb_content, 36 * i, 36, ett_xplane_data, NULL, "DATA Index: %d", index);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_index, tvb_content, (36 * i), 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_a, tvb_content, (36 * i) + 4, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_b, tvb_content, (36 * i) + 8, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_c, tvb_content, (36 * i) + 12, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_d, tvb_content, (36 * i) + 16, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_e, tvb_content, (36 * i) + 20, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_f, tvb_content, (36 * i) + 24, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_g, tvb_content, (36 * i) + 28, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_data_h, tvb_content, (36 * i) + 32, 4, ENC_LITTLE_ENDIAN);
    }
    return tvb_captured_length(tvb);
}
...
static int dissect_xplane_dcoc(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    ...
    proto_item* xplane_dcoc_item;
    proto_tree* xplane_dcoc_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 4;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.dcoc");

    xplane_dcoc_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_dcoc_item, ": Enable Data Row in Cockpit");

    xplane_dcoc_tree = proto_item_add_subtree(xplane_dcoc_item, ett_xplane_dcoc);
    proto_item* xplane_header_item = proto_tree_add_item(xplane_dcoc_tree, hf_xplane_dcoc_header, tvb, 0, 4, ENC_ASCII);
    proto_item_append_text(xplane_header_item, ", Record Count: %d", recordCount);

    for (guint32 i = 0; i < recordCount; i++)
    {
        proto_tree_add_item(xplane_dcoc_tree, hf_xplane_dcoc_id, tvb_content, i * 4, 4, ENC_LITTLE_ENDIAN);
    }
...
{
    if (tvb_captured_length(tvb) != xplane_DREF_PACKET_LENGTH)
        return 0;

    proto_item* xplane_dref_item;
    proto_tree* xplane_dref_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.dref");

    xplane_dref_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_dref_item, ": Send/Receive Dataref");

    xplane_dref_tree = proto_item_add_subtree(xplane_dref_item, ett_xplane_dref);
    ...
    proto_tree_add_item(xplane_dref_tree, hf_xplane_dref_dataref, tvb_content, 4, -1, ENC_ASCII);
...
static int dissect_xplane_dsel(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    ...
    proto_item* xplane_dsel_item;
    proto_tree* xplane_dsel_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 4;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.dsel");

    xplane_dsel_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_dsel_item, ": Enable Data Row To UDP Output");

    xplane_dsel_tree = proto_item_add_subtree(xplane_dsel_item, ett_xplane_dsel);
...
{
    proto_item* xplane_fail_item;
    proto_tree* xplane_fail_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.fail");

    xplane_fail_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_fail_item, ": Fail a Plane System");

    xplane_fail_tree = proto_item_add_subtree(xplane_fail_item, ett_xplane_fail);
    proto_tree_add_item(xplane_fail_tree, hf_xplane_fail_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_fail_tree, hf_xplane_fail_id, tvb_content, 0, -1, ENC_ASCII);
...
{
    proto_item* xplane_flir_in_item;
    proto_tree* xplane_flir_in_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.flir");

    xplane_flir_in_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_flir_in_item, ": Request FLIR data (FLIR output obsolete in 11.50)");

    xplane_flir_in_tree = proto_item_add_subtree(xplane_flir_in_item, ett_xplane_flir_in);
    proto_tree_add_item(xplane_flir_in_tree, hf_xplane_flir_in_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_flir_in_tree, hf_xplane_flir_in_framerate, tvb_content, 0, -1, ENC_ASCII);
...
{
    proto_item* xplane_flir_out_item;
    proto_tree* xplane_flir_out_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.flir");

    xplane_flir_out_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_flir_out_item, ": FLIR image data (FLIR output obsolete in 11.50)");

    xplane_flir_out_tree = proto_item_add_subtree(xplane_flir_out_item, ett_xplane_flir_out);
    ...
    proto_tree_add_item(xplane_flir_out_tree, hf_xplane_flir_out_frameindex, tvb_content, 4, 1, ENC_NA);
    proto_tree_add_item(xplane_flir_out_tree, hf_xplane_flir_out_framecount, tvb_content, 5, 1, ENC_NA);
    proto_tree_add_item(xplane_flir_out_tree, hf_xplane_flir_out_imagedata, tvb_content, 6, -1, ENC_ASCII);

    return tvb_captured_length(tvb);
}

static int dissect_xplane_ise4(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_ISE4_PACKET_LENGTH)
        return 0;

    proto_item* xplane_ise4_item;
    proto_tree* xplane_ise4_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.ise4");

    xplane_ise4_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_ise4_item, ": IPv4 Network Settings");

    xplane_ise4_tree = proto_item_add_subtree(xplane_ise4_item, ett_xplane_ise4);
...
static int dissect_xplane_ise6(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_ISE6_PACKET_LENGTH)
        return 0;

    proto_item* xplane_ise6_item;
    proto_tree* xplane_ise6_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.ise6");

    xplane_ise6_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_ise6_item, ": IPv6 Network Settings");

    xplane_ise6_tree = proto_item_add_subtree(xplane_ise6_item, ett_xplane_ise6);
...
{
    if (tvb_captured_length(tvb) != xplane_LSND_PACKET_LENGTH)
        return 0;

    proto_item* xplane_lsnd_item;
    proto_tree* xplane_lsnd_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.lsnd");

    xplane_lsnd_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_lsnd_item, ": Start Looping a Sound");

    xplane_lsnd_tree = proto_item_add_subtree(xplane_lsnd_item, ett_xplane_lsnd);
    proto_tree_add_item(xplane_lsnd_tree, hf_xplane_lsnd_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_lsnd_tree, hf_xplane_lsnd_index, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    ...
    proto_tree_add_item(xplane_lsnd_tree, hf_xplane_lsnd_filename, tvb_content, 12, -1, ENC_ASCII);
...
{
    proto_item* xplane_nfal_item;
    proto_tree* xplane_nfal_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.nfal");

    xplane_nfal_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_nfal_item, ": Fail a NavAid");

    xplane_nfal_tree = proto_item_add_subtree(xplane_nfal_item, ett_xplane_nfal);
    proto_tree_add_item(xplane_nfal_tree, hf_xplane_nfal_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_nfal_tree, hf_xplane_nfal_navaidcode, tvb_content, 0, -1, ENC_ASCII);
...
{
    proto_item* xplane_nrec_item;
    proto_tree* xplane_nrec_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.nrec");

    xplane_nrec_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_nrec_item, ": Recover a NavAid");

    xplane_nrec_tree = proto_item_add_subtree(xplane_nrec_item, ett_xplane_nrec);
    proto_tree_add_item(xplane_nrec_tree, hf_xplane_nrec_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_nrec_tree, hf_xplane_nrec_navaidcode, tvb_content, 0, -1, ENC_ASCII);
...
{
    if (tvb_captured_length(tvb) != xplane_OBJL_PACKET_LENGTH)
        return 0;

    proto_item* xplane_objl_item;
    proto_tree* xplane_objl_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.objl");

    xplane_objl_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_objl_item, ": Position an Object");

    xplane_objl_tree = proto_item_add_subtree(xplane_objl_item, ett_xplane_objl);
    proto_tree_add_item(xplane_objl_tree, hf_xplane_objl_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_objl_tree, hf_xplane_objl_index, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
...
{
    if (tvb_captured_length(tvb) != xplane_OBJN_PACKET_LENGTH)
        return 0;

    proto_item* xplane_objn_item;
    proto_tree* xplane_objn_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.objn");

    xplane_objn_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_objn_item, ": Load an Object");

    xplane_objn_tree = proto_item_add_subtree(xplane_objn_item, ett_xplane_objn);
    proto_tree_add_item(xplane_objn_tree, hf_xplane_objn_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_objn_tree, hf_xplane_objn_index, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(xplane_objn_tree, hf_xplane_objn_filename, tvb_content, 4, -1, ENC_ASCII);

    return tvb_captured_length(tvb);
}

static int dissect_xplane_prel(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_PREL_PACKET_LENGTH)
        return 0;

    proto_item* xplane_prel_item;
    proto_tree* xplane_prel_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.prel");

    xplane_prel_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_prel_item, ": Position Aircraft");

    xplane_prel_tree = proto_item_add_subtree(xplane_prel_item, ett_xplane_prel);
...
static int dissect_xplane_quit(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) == xplane_QUIT_PACKET_LENGTH)
        return 0;

    proto_item* xplane_quit_item;
    proto_tree* xplane_quit_tree;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.quit");

    xplane_quit_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_quit_item, ": Quit out of X-Plane");

    xplane_quit_tree = proto_item_add_subtree(xplane_quit_item, ett_xplane_quit);
...
{
    proto_item* xplane_radr_in_item;
    proto_tree* xplane_radr_in_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.radr");

    xplane_radr_in_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_radr_in_item, ": Request RADR data");

    xplane_radr_in_tree = proto_item_add_subtree(xplane_radr_in_item, ett_xplane_radr_in);
    proto_tree_add_item(xplane_radr_in_tree, hf_xplane_radr_in_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_radr_in_tree, hf_xplane_radr_in_pointcount, tvb_content, 0, -1, ENC_ASCII);
...
{
    if (((tvb_captured_length(tvb) - 5) % 13) != 0)
        return 0;

    proto_item* xplane_radr_out_item;
    proto_tree* xplane_radr_out_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 13;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.radr");

    xplane_radr_out_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_radr_out_item, ": RADR data output");

    xplane_radr_out_tree = proto_item_add_subtree(xplane_radr_out_item, ett_xplane_radr_out);
    proto_item* xplane_header_item = proto_tree_add_item(xplane_radr_out_tree, hf_xplane_radr_out_header, tvb, 0, 4, ENC_ASCII);
    proto_item_append_text(xplane_header_item, ", Record Count: %d", recordCount);

    for (guint32 i = 0; i < recordCount; i++)
    {
        proto_tree* xplane_content_tree = proto_tree_add_subtree_format(xplane_radr_out_tree, tvb_content, 13 * i, 13, ett_xplane_radr_out, NULL, "Element: %d", i);
        proto_tree_add_item(xplane_content_tree, hf_xplane_radr_out_longitude, tvb_content, (13 * i), 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_radr_out_latitude, tvb_content, (13 * i) + 4, 4, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(xplane_content_tree, hf_xplane_radr_out_precipitation, tvb_content, (13 * i) + 8, 1, ENC_NA);
        proto_tree_add_item(xplane_content_tree, hf_xplane_radr_out_height, tvb_content, (13 * i) + 9, 4, ENC_LITTLE_ENDIAN);
    }

    return tvb_captured_length(tvb);
}

static int dissect_xplane_reco(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    proto_item* xplane_reco_item;
    proto_tree* xplane_reco_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.reco");

    xplane_reco_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_reco_item, ": Recover a Plane System");

    xplane_reco_tree = proto_item_add_subtree(xplane_reco_item, ett_xplane_reco);
...
static int dissect_xplane_rese(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_RESE_PACKET_LENGTH)
        return 0;

    proto_item* xplane_rese_item;
    proto_tree* xplane_rese_tree;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.rese");

    xplane_rese_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_rese_item, ": Reset all Plane and Navaids");

    xplane_rese_tree = proto_item_add_subtree(xplane_rese_item, ett_xplane_rese);
...
{
    proto_item* xplane_rpos_item;
    proto_tree* xplane_rpos_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.rpos");

    xplane_rpos_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_rpos_item, ": Position Information Packet (IN)");

    xplane_rpos_tree = proto_item_add_subtree(xplane_rpos_item, ett_xplane_rpos_in);
    proto_tree_add_item(xplane_rpos_tree, hf_xplane_rpos_in_header, tvb, 0, 4, ENC_ASCII);
    proto_tree_add_item(xplane_rpos_tree, hf_xplane_rpos_in_frequency, tvb_content, 0, -1, ENC_ASCII);
...
{
    if (tvb_captured_length(tvb) != xplane_RPOS_OUT_PACKET_LENGTH)
        return 0;
    ...
    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.rpos");
    //    col_clear(pinfo->cinfo, COL_INFO);
    //    col_set_str(pinfo->cinfo, COL_INFO, "X-Plane (RPOS)");

    xplane_rpos_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_rpos_item, ": Position Information Packet (OUT)");

    xplane_rpos_tree = proto_item_add_subtree(xplane_rpos_item, ett_xplane_rpos_out);
...
static int dissect_xplane_rref_in(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    ...
    proto_item* xplane_rref_item;
    proto_tree* xplane_rref_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.rref");
    //    col_clear(pinfo->cinfo, COL_INFO);
    //    col_set_str(pinfo->cinfo, COL_INFO, "X-Plane (RPOS)");

    xplane_rref_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_rref_item, ": Request Dataref");

    xplane_rref_tree = proto_item_add_subtree(xplane_rref_item, ett_xplane_rref_in);
    proto_tree_add_item(xplane_rref_tree, hf_xplane_rref_in_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_rref_tree, hf_xplane_rref_in_frequency, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(xplane_rref_tree, hf_xplane_rref_in_id, tvb_content, 4, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(xplane_rref_tree, hf_xplane_rref_in_dataref, tvb_content, 8, 400, ENC_ASCII);
...
{
    if (((tvb_captured_length(tvb) - 5) % 8) != 0)
        return 0;
    ...
    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 8;
    ...
    xplane_rref_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_rref_item, ": Dataref Array");

    xplane_rref_tree = proto_item_add_subtree(xplane_rref_item, ett_xplane_rref_out);
...
static int dissect_xplane_shut(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_SHUT_PACKET_LENGTH)
        return 0;

    proto_item* xplane_shut_item;
    proto_tree* xplane_shut_tree;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.shut");

    xplane_shut_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_shut_item, ": Shut down the X-Plane Computer");

    xplane_shut_tree = proto_item_add_subtree(xplane_shut_item, ett_xplane_shut);
...
{
    proto_item* xplane_simo_item;
    proto_tree* xplane_simo_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.simo");

    xplane_simo_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_simo_item, ": Load/Save a Movie/Situation");

    xplane_simo_tree = proto_item_add_subtree(xplane_simo_item, ett_xplane_simo);
    proto_tree_add_item(xplane_simo_tree, hf_xplane_simo_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_simo_tree, hf_xplane_simo_action, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(xplane_simo_tree, hf_xplane_simo_filename, tvb_content, 4, -1, ENC_ASCII);
...
{
    if (tvb_captured_length(tvb) != xplane_SOUN_PACKET_LENGTH)
        return 0;

    proto_item* xplane_soun_item;
    proto_tree* xplane_soun_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.soun");

    xplane_soun_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_soun_item, ": Play a Sound");

    xplane_soun_tree = proto_item_add_subtree(xplane_soun_item, ett_xplane_soun);
    ...
    proto_tree_add_item(xplane_soun_tree, hf_xplane_soun_filename, tvb_content, 8, -1, ENC_ASCII);
...
{
    if (tvb_captured_length(tvb) != xplane_SSND_PACKET_LENGTH)
        return 0;

    proto_item* xplane_ssnd_item;
    proto_tree* xplane_ssnd_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.ssnd");

    xplane_ssnd_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_ssnd_item, ": Stop Looping a Sound");

    xplane_ssnd_tree = proto_item_add_subtree(xplane_ssnd_item, ett_xplane_ssnd);
    proto_tree_add_item(xplane_ssnd_tree, hf_xplane_ssnd_header, tvb, 0, 4, ENC_ASCII);

    proto_tree_add_item(xplane_ssnd_tree, hf_xplane_ssnd_index, tvb_content, 0, 4, ENC_LITTLE_ENDIAN);
    ...
    proto_tree_add_item(xplane_ssnd_tree, hf_xplane_ssnd_filename, tvb_content, 12, -1, ENC_ASCII);

    return tvb_captured_length(tvb);
}

static int dissect_xplane_ucoc(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (((tvb_captured_length(tvb) - 5) % 4) != 0)
        return 0;

    proto_item* xplane_ucoc_item;
    proto_tree* xplane_ucoc_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 4;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.ucoc");

    xplane_ucoc_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_ucoc_item, ": Disable Data Row in Cockpit");

    xplane_ucoc_tree = proto_item_add_subtree(xplane_ucoc_item, ett_xplane_ucoc);
...
static int dissect_xplane_usel(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (((tvb_captured_length(tvb) - 5) % 4) != 0)
        return 0;

    proto_item* xplane_usel_item;
    proto_tree* xplane_usel_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);
    guint recordCount = tvb_captured_length(tvb_content) / 4;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.usel");

    xplane_usel_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_usel_item, ": Disable Data Row To UDP Output");

    xplane_usel_tree = proto_item_add_subtree(xplane_usel_item, ett_xplane_usel);
...
static int dissect_xplane_vehx(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    if (tvb_captured_length(tvb) != xplane_VEHX_PACKET_LENGTH)
        return 0;

    proto_item* xplane_vehx_item;
    proto_tree* xplane_vehx_tree;

    tvbuff_t* tvb_content = tvb_new_subset_length(tvb, 5, tvb_reported_length(tvb) - 5);

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "xplane.vehx");

    xplane_vehx_item = proto_tree_add_item(tree, proto_xplane, tvb, 0, -1, ENC_NA);
    proto_item_append_text(xplane_vehx_item, ": Position Aircraft");

    xplane_vehx_tree = proto_item_add_subtree(xplane_vehx_item, ett_xplane_vehx);
...
{
    if (tvb_reported_length(tvb) < xplane_MIN_PACKET_LENGTH)
        return 0;

    if (tvb_captured_length(tvb) < xplane_MIN_PACKET_LENGTH)
        return 0;
...
void proto_register_xplane(void)
{
    ...
    {
        { &hf_xplane_acfn_header,   { "Header",     "xplane.acfn",          FT_STRINGZ, STR_ASCII,  NULL,   0,  "ACFN Header",  HFILL}},
        { &hf_xplane_acfn_index,    { "Index",      "xplane.acfn.index",    FT_INT32,   BASE_DEC,   NULL,   0,  "Aircraft Index (0=Own Plane, 1->19 = AI Plane.",        HFILL}},
        { &hf_xplane_acfn_path,     { "Path",       "xplane.acfn.path",     FT_STRINGZ, STR_ASCII,  NULL,   0,  "The path relative to X-Plane's home folder. Use Unix style / directory seperators.",         HFILL}},
        { &hf_xplane_acfn_padding,  { "Padding",    "xplane.acfn.padding",  FT_BYTES,   BASE_NONE,  NULL,   0,  "Padding",      HFILL}},
    ...
    {
        { &hf_xplane_acpr_header,           { "Header",             "xplane.acpr",                  FT_STRINGZ, STR_ASCII,  NULL,   0,  "ACPR Header",      HFILL}},
        { &hf_xplane_acpr_index,            { "Index",              "xplane.acpr.index",            FT_INT32,   BASE_DEC,   NULL,   0,  "Aircraft Index (0=Own Plane, 1->19 = AI Plane.",            HFILL}},
        { &hf_xplane_acpr_path,             { "Path",               "xplane.acpr.path",             FT_STRINGZ, STR_ASCII,  NULL,   0,  "The path relative to X-Plane's home folder. Use Unix style / directory seperators.",             HFILL}},
        { &hf_xplane_acpr_padding,          { "Padding",            "xplane.acpr.padding",          FT_BYTES,   BASE_NONE,  NULL,   0,  "Padding",          HFILL}},
        ...
        { &hf_xplane_acpr_starttype,        { "Start Type",         "xplane.acpr.starttype",        FT_INT32,   BASE_DEC,   VALS(xplane_StartTypeLookup),   0,  "The Start type such as runway, LatLong, etc.",       HFILL}},
        ...
        { &hf_xplane_acpr_ICAO,             { "ICAO",               "xplane.acpr.ICAO",             FT_STRING,  STR_ASCII,  NULL,   0,  "Airport / NavAid text to place aircraft at. Note: Max 7 chars as the packet's struct is 8 chars including the ending '\0'.",             HFILL}},
        ...
        { &hf_xplane_acpr_trueheading,      { "True Heading",       "xplane.acpr.trueheading",      FT_DOUBLE,  BASE_NONE,  NULL,   0,  "True Heading",     HFILL}},
    ...
    {
        { &hf_xplane_alrt_header,   { "Header",         "xplane.alrt",          FT_STRINGZ, STR_ASCII,  NULL,   0,  "ALRT Header",  HFILL}},
        { &hf_xplane_alrt_line1,    { "Line 1",         "xplane.alrt.line1",    FT_STRINGZ, STR_ASCII,  NULL,   0,  "Line 1",       HFILL}},
        { &hf_xplane_alrt_line2,    { "Line 2",         "xplane.alrt.line2",    FT_STRINGZ, STR_ASCII,  NULL,   0,  "Line 2",       HFILL}},
        { &hf_xplane_alrt_line3,    { "Line 3",         "xplane.alrt.line3",    FT_STRINGZ, STR_ASCII,  NULL,   0,  "Line 3",       HFILL}},
        { &hf_xplane_alrt_line4,    { "Line 4",         "xplane.alrt.line4",    FT_STRINGZ, STR_ASCII,  NULL,   0,  "Line 4",       HFILL}}
    };
    static hf_register_info hf_xplane_becn[] =
    {
        ...
        { &hf_xplane_becn_major,    { "Major Version",  "xplane.becn.major",    FT_UINT8,   BASE_DEC,   NULL,   0,  "Major Version",    HFILL}},
        { &hf_xplane_becn_minor,    { "Minor Version",  "xplane.becn.minor",    FT_UINT8,   BASE_DEC,   NULL,   0,  "Minor Version",    HFILL}},
        { &hf_xplane_becn_hostid,   { "Host ID",        "xplane.becn.hostid",   FT_INT32,   BASE_DEC,   NULL,   0,  "Host ID",          HFILL}},
        { &hf_xplane_becn_version,  { "Version",        "xplane.becn.version",  FT_INT32,   BASE_DEC,   NULL,   0,  "Version Number",   HFILL}},
        { &hf_xplane_becn_role,     { "Role",           "xplane.becn.role",     FT_UINT32,  BASE_DEC,   VALS(xplane_BeaconTypeLookup), 0,"Role", HFILL}},
        { &hf_xplane_becn_port,     { "Port",           "xplane.becn.port",     FT_UINT32,  BASE_DEC,   NULL,   0,  "Port",             HFILL}},
        { &hf_xplane_becn_name,     { "Computer name",  "xplane.becn.name",     FT_STRINGZ, STR_ASCII,  NULL,   0,  "Computer Name",    HFILL}},
        { &hf_xplane_becn_newport,  { "New Port",       "xplane.becn.newport",  FT_UINT16,  BASE_DEC,   NULL,   0,  "New Port",         HFILL}}
    };
    static hf_register_info hf_xplane_cmnd[] =
    {
        ...
        { &hf_xplane_cmnd_command,  { "Command",    "xplane.cmnd.command",  FT_STRINGZ, STR_ASCII,  NULL,   0,  "Command",      HFILL}}
    };
    ...
    static hf_register_info hf_xplane_dcoc[] =
    {
        ...
        { &hf_xplane_dcoc_id,       { "Id",     "xplane.dcoc.id",   FT_INT32,   BASE_DEC,   NULL,   0,  "Id",      HFILL}}
    };
    static hf_register_info hf_xplane_dref[] =
    {
        ...
        { &hf_xplane_dref_value,    { "Value",      "xplane.dref.value",    FT_FLOAT,   BASE_NONE,  NULL,   0,  "Value",        HFILL}},
        { &hf_xplane_dref_dataref,  { "Dataref",    "xplane.dref.dataref",  FT_STRINGZ, STR_ASCII,  NULL,   0,  "Dataref",      HFILL}}
    };
    static hf_register_info hf_xplane_dsel[] =
    {
        ...
        { &hf_xplane_dsel_id,       { "Id",     "xplane.dsel.id",   FT_INT32,   BASE_DEC,   NULL,   0,  "Id",      HFILL}}
    };
    static hf_register_info hf_xplane_fail[] =
    {
        ...
        { &hf_xplane_fail_id,       { "Id",     "xplane.fail.id",   FT_STRINGZ, STR_ASCII,  NULL,   0,  "Id",      HFILL}}
    };
    static hf_register_info hf_xplane_flir_in[] =
    {
        ...
        { &hf_xplane_flir_in_framerate,     { "Frame Rate",     "xplane.flir.framerate",    FT_INT32,   BASE_DEC,   NULL,   0,  "Frame Rate",   HFILL}}
    };
    static hf_register_info hf_xplane_flir_out[] =
    {
        ...
        { &hf_xplane_flir_out_frameindex,   { "Frame Index",    "xplane.flir.frameindex",   FT_UINT8, BASE_DEC,     NULL,   0,  "Frame Index",          HFILL}},
        { &hf_xplane_flir_out_framecount,   { "Frame Count",    "xplane.flir.framecount",   FT_UINT8, BASE_DEC,     NULL,   0,  "Frame Count",          HFILL}},
        { &hf_xplane_flir_out_imagedata,    { "Image Date",     "xplane.flir.imagedata",    FT_BYTES, BASE_NONE,    NULL,   0,  "Image Data",           HFILL}}
    };
    static hf_register_info hf_xplane_ise4[] =
    {
        ...
        { &hf_xplane_ise4_machinetype,  { "Machine Type",   "xplane.ise4.machinetype",  FT_INT32,   BASE_DEC,   NULL,   0,  "Machine Type", HFILL}},
        { &hf_xplane_ise4_address,      { "Address",        "xplane.ise4.address",      FT_STRINGZ, STR_ASCII,  NULL,   0,  "Address",      HFILL}},
        { &hf_xplane_ise4_port,         { "Port",           "xplane.ise4.port",         FT_STRINGZ, STR_ASCII,  NULL,   0,  "Port",         HFILL}},
        { &hf_xplane_ise4_enabled,      { "Enabled",        "xplane.ise4.enabled",      FT_INT32,   BASE_DEC,   NULL,   0,  "Enabled",      HFILL}}
    };
    static hf_register_info hf_xplane_ise6[] =
    {
        ...
        { &hf_xplane_ise6_machinetype,  { "Machine Type",   "xplane.ise6.machinetype",  FT_INT32,   BASE_DEC,   NULL,   0,  "Machine Type", HFILL}},
        { &hf_xplane_ise6_address,      { "Address",        "xplane.ise6.address",      FT_STRING,  STR_ASCII,  NULL,   0,  "Address",      HFILL}},
        { &hf_xplane_ise6_port,         { "Port",           "xplane.ise6.port",         FT_STRING,  STR_ASCII,  NULL,   0,  "Port",         HFILL}},
        { &hf_xplane_ise6_enabled,      { "Enabled",        "xplane.ise6.enabled",      FT_INT32,   BASE_DEC,   NULL,   0,  "Enabled",      HFILL}}
    };
    static hf_register_info hf_xplane_lsnd[] =
    {
        ...
        { &hf_xplane_lsnd_index,    { "Index",      "xplane.lsnd.index",    FT_INT32,   BASE_DEC,   NULL,   0,  "Index",        HFILL}},
        { &hf_xplane_lsnd_speed,    { "Speed",      "xplane.lsnd.speed",    FT_FLOAT,   BASE_NONE,  NULL,   0,  "Speed",        HFILL}},
        { &hf_xplane_lsnd_volume,   { "Volume",     "xplane.lsnd.volume",   FT_FLOAT,   BASE_NONE,  NULL,   0,  "Volume",       HFILL}},
        { &hf_xplane_lsnd_filename, { "Filename",   "xplane.lsnd.filename", FT_STRING,  STR_ASCII,  NULL,   0,  "Filename",     HFILL}}
    };
    static hf_register_info hf_xplane_nfal[] =
    {
        ...
        { &hf_xplane_nfal_navaidcode,   { "Navaid Code",    "xplane.nfal.id",   FT_STRINGZ, STR_ASCII,  NULL,   0,  "Nav Aid Code", HFILL}},
    };
    static hf_register_info hf_xplane_nrec[] =
    {
        ...
        { &hf_xplane_nrec_navaidcode,   { "Navaid Code",    "xplane.nrec.id",   FT_STRINGZ, STR_ASCII,  NULL,   0,  "Nav Aid Code", HFILL}},
    };
    static hf_register_info hf_xplane_objl[] =
    {
        ...
        { &hf_xplane_objl_index,        { "Index",      "xplane.objl.index",        FT_INT32,   BASE_DEC,    NULL,   0,  "Index",        HFILL}},
        { &hf_xplane_objl_padding1,     { "Padding 1",  "xplane.objl.padding1",     FT_BYTES,   BASE_NONE,   NULL,   0,  "Padding 1",    HFILL}},
        { &hf_xplane_objl_latitude,     { "Latitude",   "xplane.objl.latitude",     FT_DOUBLE,  BASE_NONE,   NULL,   0,  "Latitude",     HFILL}},
        { &hf_xplane_objl_longitude,    { "Longitude",  "xplane.objl.longitude",    FT_DOUBLE,  BASE_NONE,   NULL,   0,  "Longitude",    HFILL}},
        { &hf_xplane_objl_elevation,    { "Elevation",  "xplane.objl.elevation",    FT_DOUBLE,  BASE_NONE,   NULL,   0,  "Elevation",    HFILL}},
        { &hf_xplane_objl_psi,          { "Psi",        "xplane.objl.psi",          FT_FLOAT,   BASE_NONE,   NULL,   0,  "Psi",          HFILL}},
        { &hf_xplane_objl_theta,        { "Theta",      "xplane.objl.theta",        FT_FLOAT,   BASE_NONE,   NULL,   0,  "Theta",        HFILL}},
        { &hf_xplane_objl_phi,          { "Phi",        "xplane.objl.phi",          FT_FLOAT,   BASE_NONE,   NULL,   0,  "Phi",          HFILL}},
        { &hf_xplane_objl_onground,     { "Onground ",  "xplane.objl.onground",     FT_INT32,   BASE_DEC,    NULL,   0,  "Onground",     HFILL}},
        { &hf_xplane_objl_smokesize,    { "Smokesize",  "xplane.objl.smokesize",    FT_FLOAT,   BASE_NONE,   NULL,   0,  "Smokesize",    HFILL}},
        { &hf_xplane_objl_padding2,     { "Padding 2",  "xplane.objl.padding2",     FT_BYTES,   BASE_NONE,   NULL,   0,  "Padding 2",    HFILL}}
    };
    static hf_register_info hf_xplane_objn[] =
    {
        ...
        { &hf_xplane_objn_index,        { "Index",      "xplane.objn.index",        FT_INT32,   BASE_DEC,   NULL,   0,  "Index",        HFILL}},
        { &hf_xplane_objn_filename,     { "Filename",   "xplane.objn.filename",     FT_STRINGZ, STR_ASCII,  NULL,   0,  "Filename",     HFILL}},
    };
    static hf_register_info hf_xplane_prel[] =
    {
        ...
        { &hf_xplane_prel_starttype,        { "Start Type",         "xplane.prel.starttype",        FT_INT32,   BASE_DEC,   VALS(xplane_StartTypeLookup),   0,  "Start Type",       HFILL}},
        { &hf_xplane_prel_aircraftindex,    { "Aircraft Index",     "xplane.prel.aircraftindex",    FT_INT32,   BASE_DEC,   NULL,   0,  "Aircraft Index",   HFILL}},
        { &hf_xplane_prel_ICAO,             { "ICAO",               "xplane.prel.ICAO",             FT_STRING,  STR_ASCII,  NULL,   0,  "ICAO",             HFILL}},
        { &hf_xplane_prel_runwayindex,      { "Runway Index",       "xplane.prel.runwayindex",      FT_INT32,   BASE_DEC,   NULL,   0,  "Runway Index",     HFILL}},
        { &hf_xplane_prel_runwaydirection,  { "Runway Direction",   "xplane.prel.runwaydirection",  FT_INT32,   BASE_DEC,   NULL,   0,  "Runway Direction", HFILL}},
        { &hf_xplane_prel_latitude,         { "Latitude",           "xplane.prel.latitude",         FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Latitude",         HFILL}},
        { &hf_xplane_prel_longitude,        { "Longitude",          "xplane.prel.longitude",        FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Longitude",        HFILL}},
        { &hf_xplane_prel_elevation,        { "Elevation",          "xplane.prel.elevation",        FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Elevation",        HFILL}},
        { &hf_xplane_prel_trueheading,      { "True Heading",       "xplane.prel.trueheading",      FT_DOUBLE,  BASE_NONE,  NULL,   0,  "True Heading",     HFILL}},
        { &hf_xplane_prel_speed,            { "Speed",              "xplane.prel.speed",            FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Speed",            HFILL}},
    };
    ...
    static hf_register_info hf_xplane_radr_in[] =
    {
        ...
        { &hf_xplane_radr_in_pointcount,    { "Points Per Frame",   "xplane.radr.id",   FT_STRINGZ, STR_ASCII,  NULL,   0,  "Points Per Frame", HFILL}},
    };
    static hf_register_info hf_xplane_radr_out[] =
    {
        ...
        { &hf_xplane_radr_out_longitude,     { "Longitude",     "xplane.radr.longitude",        FT_FLOAT,   BASE_NONE,  NULL,   0,  "Longitude",            HFILL}},
        { &hf_xplane_radr_out_latitude,      { "Latitude",      "xplane.radr.latitude",         FT_FLOAT,   BASE_NONE,  NULL,   0,  "Latitude",             HFILL}},
        { &hf_xplane_radr_out_precipitation, { "Precipitation", "xplane.radr.precipitation",    FT_INT8,    BASE_DEC,   NULL,   0,  "Precipitation",        HFILL}},
        { &hf_xplane_radr_out_height,        { "Storm Height",  "xplane.radr.height",           FT_FLOAT,   BASE_NONE,  NULL,   0,  "Height",               HFILL}},
    };
    static hf_register_info hf_xplane_reco[] =
    {
        ...
        { &hf_xplane_reco_id,       { "Id",     "xplane.reco.id",   FT_STRINGZ, STR_ASCII,  NULL,   0,  "Id",           HFILL}}
    };
    ...
    static hf_register_info hf_xplane_rpos_in[] =
    {
        ...
        { &hf_xplane_rpos_in_frequency,     { "Frequency",  "xplane.rpos.frequency",    FT_STRINGZ, STR_ASCII,  NULL,   0,  "Frequency",  HFILL}},
    };
    static hf_register_info hf_xplane_rpos_out[] =
    {
        ...
        { &hf_xplane_rpos_out_longitude,    { "Longitude",  "xplane.rpos.longitude",    FT_DOUBLE,   BASE_NONE,  NULL,   0,  "Longitude",                               HFILL}},
        { &hf_xplane_rpos_out_latitude ,    { "Latitude ",  "xplane.rpos.latitude",     FT_DOUBLE,   BASE_NONE,  NULL,   0,  "Latitude ",                               HFILL}},
        { &hf_xplane_rpos_out_elevation,    { "Elevation",  "xplane.rpos.elevation",    FT_DOUBLE,   BASE_NONE,  NULL,   0,  "Altitude (metres above mean sea level)",  HFILL}},
        { &hf_xplane_rpos_out_height,       { "Height",     "xplane.rpos.height",       FT_FLOAT,    BASE_NONE,  NULL,   0,  "Height (metres above ground)",            HFILL}},
        { &hf_xplane_rpos_out_theta,        { "Theta",      "xplane.rpos.theta",        FT_FLOAT,    BASE_NONE,  NULL,   0,  "Pitch (degrees)",                         HFILL}},
        { &hf_xplane_rpos_out_psi,          { "Psi",        "xplane.rpos.psi",          FT_FLOAT,    BASE_NONE,  NULL,   0,  "True Heading (degrees)",                  HFILL}},
        { &hf_xplane_rpos_out_phi,          { "Phi",        "xplane.rpos.phi",          FT_FLOAT,    BASE_NONE,  NULL,   0,  "Roll (degrees)",                          HFILL}},
        { &hf_xplane_rpos_out_vx,           { "Vx",         "xplane.rpos.vx",           FT_FLOAT,    BASE_NONE,  NULL,   0,  "Velocity (East)",                         HFILL}},
        { &hf_xplane_rpos_out_vy,           { "Vy",         "xplane.rpos.vy",           FT_FLOAT,    BASE_NONE,  NULL,   0,  "Velocity (Vertical)",                     HFILL}},
        { &hf_xplane_rpos_out_vz,           { "Vz",         "xplane.rpos.vz",           FT_FLOAT,    BASE_NONE,  NULL,   0,  "Velocity (South)",                        HFILL}},
        { &hf_xplane_rpos_out_rollrate,     { "Rollrate",   "xplane.rpos.rollrate",     FT_FLOAT,    BASE_NONE,  NULL,   0,  "Roll Rate",                               HFILL}},
        { &hf_xplane_rpos_out_pitchrate,    { "Pitchrate",  "xplane.rpos.pitchrate",    FT_FLOAT,    BASE_NONE,  NULL,   0,  "Pitch Rate",                              HFILL}},
        { &hf_xplane_rpos_out_yawrate,      { "Yawrate",    "xplane.rpos.yawrate",      FT_FLOAT,    BASE_NONE,  NULL,   0,  "Yaw Rate",                                HFILL}}
    };
    static hf_register_info hf_xplane_rref_in[] =
    {
        ...
        { &hf_xplane_rref_in_frequency, { "Frequency",  "xplane.rref.frequency",    FT_INT32,   BASE_DEC,   NULL,   0,  "Frequency",        HFILL}},
        { &hf_xplane_rref_in_id,        { "Id",         "xplane.rref.id",           FT_INT32,   BASE_DEC,   NULL,   0,  "ID",               HFILL}},
        { &hf_xplane_rref_in_dataref,   { "Dataref",    "xplane.rref.dataref",      FT_STRINGZ, STR_ASCII,  NULL,   0,  "Dataref",          HFILL}}
    };
    static hf_register_info hf_xplane_rref_out[] =
    {
        ...
        { &hf_xplane_rref_out_id,       { "Id",     "xplane.rref.id",       FT_INT32,   BASE_DEC,   NULL,   0,  "ID",                   HFILL}},
        { &hf_xplane_rref_out_value,    { "Value",  "xplane.rref.value",    FT_FLOAT,   BASE_NONE,  NULL,   0,  "Value",                HFILL}}
    };
    ...
    static hf_register_info hf_xplane_simo[] =
    {
        ...
        { &hf_xplane_simo_action,   { "Header", "xplane.simo.action",   FT_INT32,   BASE_DEC,   NULL,   0,  "Action",       HFILL}},
        { &hf_xplane_simo_filename, { "Header", "xplane.simo.filename", FT_STRINGZ, STR_ASCII,  NULL,   0,  "Filename",     HFILL}}
    };
    static hf_register_info hf_xplane_soun[] =
    {
        ...
        { &hf_xplane_soun_speed,    { "Speed",      "xplane.soun.speed",    FT_FLOAT,   BASE_NONE,  NULL,   0,  "Speed",        HFILL}},
        { &hf_xplane_soun_volume,   { "Volume",     "xplane.soun.volume",   FT_FLOAT,   BASE_NONE,  NULL,   0,  "Volume",       HFILL}},
        { &hf_xplane_soun_filename, { "Filename",   "xplane.soun.filename", FT_STRING,  STR_ASCII,  NULL,   0,  "Filename",     HFILL}}
    };
    static hf_register_info hf_xplane_ssnd[] =
    {
        ...
        { &hf_xplane_ssnd_index,    { "Index",      "xplane.ssnd.index",    FT_INT32,   BASE_DEC,   NULL,   0,  "Index",        HFILL}},
        { &hf_xplane_ssnd_speed,    { "Speed",      "xplane.ssnd.speed",    FT_FLOAT,   BASE_NONE,  NULL,   0,  "Speed",        HFILL}},
        { &hf_xplane_ssnd_volume,   { "Volume",     "xplane.ssnd.volume",   FT_FLOAT,   BASE_NONE,  NULL,   0,  "Volume",       HFILL}},
        { &hf_xplane_ssnd_filename, { "Filename",   "xplane.ssnd.filename", FT_STRING,  STR_ASCII,  NULL,   0,  "Filename",     HFILL}}
    };
    static hf_register_info hf_xplane_ucoc[] =
    {
        ...
        { &hf_xplane_ucoc_id,       { "Id",     "xplane.ucoc.id",   FT_INT32,   BASE_DEC,   NULL,   0,  "Id",           HFILL}}
    };
    static hf_register_info hf_xplane_usel[] =
    {
        ...
        { &hf_xplane_usel_id,       { "Id",     "xplane.usel.id",   FT_INT32,   BASE_DEC,   NULL,   0,  "Id",           HFILL}}
    };
    static hf_register_info hf_xplane_vehx[] =
    {
        ...
        { &hf_xplane_vehx_id,           { "Id",         "xplane.vehx.id",           FT_INT32,   BASE_DEC,   NULL,   0,  "Id",           HFILL}},
        { &hf_xplane_vehx_latitude,     { "Latitude",   "xplane.vehx.latitude",     FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Latitude",     HFILL}},
        { &hf_xplane_vehx_longitude,    { "Longitude",  "xplane.vehx.longitude",    FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Longitude",    HFILL}},
        { &hf_xplane_vehx_elevation,    { "Elevation",  "xplane.vehx.elevation",    FT_DOUBLE,  BASE_NONE,  NULL,   0,  "Elevation",    HFILL}},
        { &hf_xplane_vehx_heading,      { "Heading",    "xplane.vehx.heading",      FT_FLOAT,   BASE_NONE,  NULL,   0,  "Heading",      HFILL}},
        { &hf_xplane_vehx_pitch,        { "Pitch",      "xplane.vehx.pitch",        FT_FLOAT,   BASE_NONE,  NULL,   0,  "Pitch",        HFILL}},
        { &hf_xplane_vehx_roll,         { "Roll",       "xplane.vehx.roll",         FT_FLOAT,   BASE_NONE,  NULL,   0,  "Roll",         HFILL}}
    };
